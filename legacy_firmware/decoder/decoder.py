...
####################
# properties of the 45 thermistors + mlx packets
_BD_45THERM1MLX_MSG_FIXED_LENGTH = 3  # start byte, byte metadata nbr packets, byte end
_BD_45THERM1MLX_MSG_NBR_THERMISTORS = 45
_BD_45THERM1MLX_PACKET_NBR_BYTES_PER_THERMISTOR = 3
_BD_45THERM1MLX_PACKET_LENGTH = 1 + 4 + 5 + _BD_45THERM1MLX_PACKET_NBR_BYTES_PER_THERMISTOR * _BD_45THERM1MLX_MSG_NBR_THERMISTORS + 0
# these are from the params of the tracker
# _BD_THERM_ROLL_FLOAT_TO_INT8_FACTOR = 0.7
# _BD_THERM_PITCH_FLOAT_TO_INT8_FACTOR = 1.4
# factor for converting bin 12 bits signed int to temperature; from thermistor datasheet
...
def message_kind(bin_msg):
    ...
    valid_first_chars = ["G", "Y", "T", "U", "V", "W", "A"]
...
def decode_45thermistors1mlx_packet(bin_packet, print_decoded=False, print_debug_information=False):
    if print_debug_information:
        print("----- START DECODE THERMMLX PACKET -----")

    # print(len(bin_packet))
    # print(_BD_45THERM1MLX_PACKET_LENGTH)
    assert len(bin_packet) == _BD_45THERM1MLX_PACKET_LENGTH
    ...
    for crrt_thermistor in range(_BD_45THERM1MLX_MSG_NBR_THERMISTORS):
        crrt_thermistor_bin = bin_packet[crrt_start_field: crrt_start_field+3]
        crrt_start_field += 3
        crrt_thermistor_reading = decode_thermistor_reading(crrt_thermistor_bin, print_debug_information=print_debug_information)
        assert isinstance(crrt_thermistor_reading, Thermistors_Reading)
    ...
    # print(crrt_start_field)
    # print(_BD_45THERM1MLX_PACKET_LENGTH)
    assert crrt_start_field == _BD_45THERM1MLX_PACKET_LENGTH
...
def decode_45thermistors1mlx_message(bin_msg, print_decoded=False, print_debug_information=False):
    if print_decoded:
        print("----------------------- START DECODE THERMISTORS MESSAGE -----------------------")

    assert message_kind(bin_msg) == "A"
    assert byte_to_char(bin_msg[-1]) == "E"

    # print(len(bin_msg))
    # print(_BD_45THERM1MLX_MSG_FIXED_LENGTH)
    # print(_BD_45THERM1MLX_PACKET_LENGTH)
    expected_message_length = int( (len(bin_msg) - _BD_45THERM1MLX_MSG_FIXED_LENGTH) / _BD_45THERM1MLX_PACKET_LENGTH )

    if (print_debug_information):
        print("received message of length: {}".format(len(bin_msg)))
        print("number of packets inside (expected_message_length): {}".format(expected_message_length))
        print("total expected message length: {}".format(expected_message_length * _BD_45THERM1MLX_PACKET_LENGTH + _BD_45THERM1MLX_MSG_FIXED_LENGTH))

    # print(expected_message_length * _BD_45THERM1MLX_PACKET_LENGTH + _BD_45THERM1MLX_MSG_FIXED_LENGTH)
    # print(len(bin_msg))
    assert expected_message_length * _BD_45THERM1MLX_PACKET_LENGTH + _BD_45THERM1MLX_MSG_FIXED_LENGTH == len(bin_msg)
    ...
    while True:
        ...
        # decode
        crrt_decoded_packet = decode_45thermistors1mlx_packet(bin_msg[crrt_packet_start: crrt_packet_start+_BD_45THERM1MLX_PACKET_LENGTH], print_decoded=print_decoded, print_debug_information=print_debug_information)
        list_decoded_packets.append(crrt_decoded_packet)

        trailing_char = byte_to_char(bin_msg[crrt_packet_start + _BD_45THERM1MLX_PACKET_LENGTH])
        # print(trailing_char)
        assert trailing_char in ["P", "E"]
        if trailing_char == "E":
            break
        else:
            crrt_packet_start += _BD_45THERM1MLX_PACKET_LENGTH
...
def decode_message(hex_string_message, print_decoded=True, print_debug_information=False):
    ...
    if kind == "G":
        ...
        message_metadata, list_decoded_packets = decode_45thermistors1mlx_message(bin_msg, print_decoded=print_decoded, print_debug_information=print_debug_information)
