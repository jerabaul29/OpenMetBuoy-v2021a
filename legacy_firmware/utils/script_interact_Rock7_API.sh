...
if [[ "${FILE_SOURCE}" == "apirequest" ]]; then
  ...
  curl "https://rockblock.rock7.com/RockBlockAdmin?filterDeviceAssignmentId=&filterDirection=&filterDateFrom=06%2FNov%2F2021+00%3A00%3A00&filterDateTo=30%2FJun%2F2022+23%3A59%3A59&format=csv&page=messages&action=getMessages" \
    -H 'Connection: keep-alive' \
    -H 'Pragma: no-cache' \
    -H 'Cache-Control: no-cache' \
...
if [[ "${IS_VALID}" == "TRUE" ]]; then
  icp "generate all messages..."
  python3 script_all_messages_to_dict.py
  icp "generate map figs..."
  python3 script_plot_trajectories_on_map.py
  icp "generate waves figs..."
  python3 script_plot_spectra.py
fi
