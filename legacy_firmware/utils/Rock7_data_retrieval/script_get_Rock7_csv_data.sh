...
if [[ "${FILE_SOURCE}" == "apirequest" ]]; then
  ...
  # NOTE: the string below contains the time bounds: filterDateFrom=22%2FJul%2F2022+00%3A00%3A00&filterDateTo=30%2FJun%2F2033+23%3A59%3A59 ; edit as needed
  # NOTE: it is possible to grab only a specific buoy by setting filterDeviceAssignmentId= ; for example: filterDeviceAssignmentId=297241 ; note that the ID is neither the name, not the serial number, nor the IMEI provided in the Rock7 interface; to determine it, do a request by hand and track the string emitted by your web browser with the debugging tool
